...
/**
 * Naimportuje {@code dataset/*.json} (zabalené v jaru, viz
 * {@code pom.xml}/resources) do Postgresu. Volá se z {@link DatasetImporter}
 * při startu aplikace.
 *
 ...
 * {@code docker compose down -v} - viz BACKEND_ZAPISNIK.md. Skutečné
 * verzování dat (víc verzí datasetu vedle sebe v jedné databázi) je mimo
 * rozsah slice; jedna hra, jeden aktuální dataset.
 *
...
public class DatasetImportService {
    ...
    public void importIfNeeded() {
        ...
        byte[] skillsBytes = readResourceBytes("dataset/skills_raw.json");
        byte[] monstersBytes = readResourceBytes("dataset/monsters_base.json");
        ...
        IndexFile index = readJson("dataset/index.json", IndexFile.class);
    ...
    private Map<String, Long> importMonsters(long gameId, MonstersBaseFile monstersFile) {
        ...
        jdbcTemplate.query(
                ...
                // vyskytu stejneho vzoru v importSkills(), nebo
                // docs/BACKEND_ZAPISNIK.md krok 3 pro cely rozbor.
    ...
    private byte[] readResourceBytes(String classpathPath) {
        try (InputStream in = new ClassPathResource(classpathPath).getInputStream()) {
            return in.readAllBytes();
        } catch (IOException e) {
            throw new DatasetImportException("Nepodarilo se precist " + classpathPath + " z classpath", e);
        }
    }

    private <T> T readJson(String classpathPath, Class<T> type) {
        return parseJson(readResourceBytes(classpathPath), type);
    }
     ...
     * (název skillu + název sloupce) - ověřeno skenem celého datasetu před
     * psaním importeru, že se to dnes nemůže stát (viz
     * docs/BACKEND_ZAPISNIK.md, krok 3); kdyby se to změnilo v budoucím
     * refreshi, chceme hlasitou chybu při startu, ne tichou nulu.
     */
